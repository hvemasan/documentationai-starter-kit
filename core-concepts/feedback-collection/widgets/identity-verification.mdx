...
<Card title="Open Widget & Embeds" href="https://app.productbridge.io/dashboard/settings/widget-embeds" icon="shield-check" horizontal="true">
  Sidebar → **Settings** → Connect → **Widget & Embeds** → **User Identity Verification**
</Card>
...
**In-app path:** Sidebar → **Settings** → Connect → **Widget & Embeds** → **Embed Code** tab → **User Identity Verification** section

**Signing key:** [https://app.productbridge.io/dashboard/settings/sso?tab=api-keys](https://app.productbridge.io/dashboard/settings/sso?tab=api-keys) — Settings → Connect → **SSO** → **API Keys**

**Requires:** `organization:manage`.

<Callout kind="alert">
  The **Widget Auth** tab that used to hold these toggles has been removed from the SSO settings page. The JWT snippet now lives in the **User Identity Verification** section of the Widget & Embeds page, and the signing key stays on [SSO → API Keys](https://app.productbridge.io/dashboard/settings/sso?tab=api-keys).
</Callout>

## Why Identify Users?

By default, users are prompted to sign in inside the widget before they can post. When you add identity verification:
...
<Steps>
  <Step title="Open Widget Settings" icon="settings" title-type="p">
    In your ProductBridge dashboard go to [**Settings → Connect → Widget & Embeds**](https://app.productbridge.io/dashboard/settings/widget-embeds) and open the **API Keys** tab.
  </Step>
...
export default function ProductBridgeWidget({
  ...
  useEffect(() => {
    ...
    fetch('/api/productbridge-token')
      .then((res) => {
        if (!res.ok) return { token: null }; // no token — users sign in inside the widget
        return res.json();
      })
      .then(({ token }) => {
...
onMounted(async () => {
  ...
  try {
    ...
    // no token — users sign in inside the widget
  }
...
export class ProductBridgeWidgetComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.http.get<{ token: string }>('/api/productbridge-token').subscribe({
      ...
      error: () => this.loadWidget(null), // no token — users sign in inside the widget
    });
  }
...
| `account_owner_id` | string | No | Your internal ID of the account manager or CSM |
...
| `company_created_at` | string | No | ISO date string of when the company account was created |
...
Your frontend should fetch a fresh token on each page load or session start. Since the token endpoint is behind your own authentication middleware, unauthenticated users automatically get no token, and the widget prompts them to sign in instead.
...
```javascript
// 1. Init without a token — users would be prompted to sign in
...
<Expandable title="Troubleshooting" default-open="false">
  **Widget shows "Please log in" even though I passed a token**
  ...
  The most common cause is a signature mismatch — the secret used to sign the JWT doesn't match the Widget API Secret stored in ProductBridge. Double-check that `PRODUCTBRIDGE_WIDGET_SECRET` on your server matches the key in the **API Keys** tab of [**Settings → Connect → Widget & Embeds**](https://app.productbridge.io/dashboard/settings/widget-embeds).
