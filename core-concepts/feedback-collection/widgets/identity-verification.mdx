...
<Steps>
  <Step title="Open Widget Settings" icon="settings" title-type="p">
    In your ProductBridge dashboard go to **Settings → Connect → Widget & Embeds** and open the **API Keys** tab.
  </Step>
...
<Expandable title="Troubleshooting" default-open="false">
  **Widget shows "Please log in" even though I passed a token**
  ...
  The most common cause is a signature mismatch — the secret used to sign the JWT doesn't match the Widget API Secret stored in ProductBridge. Double-check that `PRODUCTBRIDGE_WIDGET_SECRET` on your server matches the key in the **API Keys** tab of **Settings → Connect → Widget & Embeds**.
