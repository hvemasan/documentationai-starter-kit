...
<Card title="Open Help Center" href="https://app.productbridge.io/dashboard/help-center" icon="book" horizontal="true">
  Sidebar → **Help Center**
</Card>

**Direct link:** [https://app.productbridge.io/dashboard/help-center](https://app.productbridge.io/dashboard/help-center)

**In-app path:** Sidebar → **Help Center**

**Requires:** `helpcenter:view` to read, `helpcenter:manage` to author, and a plan that includes the knowledge base feature.

| Goal | Direct link |
|------|-------------|
| Write and organize articles | [`/dashboard/help-center`](https://app.productbridge.io/dashboard/help-center) |
| Article analytics dashboard | [`/dashboard/help-center?view=dashboard`](https://app.productbridge.io/dashboard/help-center?view=dashboard) |
| Starred articles only | [`/dashboard/help-center?favourites=1`](https://app.productbridge.io/dashboard/help-center?favourites=1) |
| Search your articles | `/dashboard/help-center?q={search+term}` |
| Filter to one collection | `/dashboard/help-center?collection={collectionId}` |
| Filter to one category | `/dashboard/help-center?category={categoryId}` |
| Filter by publish status | `/dashboard/help-center?status=draft` |
| Turn the Help tab on for your portal | [`/dashboard/settings/public-portal`](https://app.productbridge.io/dashboard/settings/public-portal) |
| Your public help center | `https://{your-subdomain}.productbridge.io/help` |

<Callout kind="info">
  Published articles live at `https://{your-subdomain}.productbridge.io/help/{collection}/{category}/{article}`. The category segment is omitted for articles filed straight under a collection.
</Callout>
...
<Tabs>
  <Tab title="Public Portal" icon="globe">
    Go to [**Settings → Public Portal**](https://app.productbridge.io/dashboard/settings/public-portal) and toggle **Help Center** on. The **Help** tab then appears in your portal navigation.
  </Tab>
...
Everything you can author in the dashboard is also available over the public API, so you can migrate content from another help desk, keep articles in sync with a docs repository, or surface your articles inside your own product.

<Columns cols="3">
  <Card title="Help collections" href="/api-reference/help-collections/overview" icon="library" horizontal="false">
    Create, update, and delete the top-level topic areas of your Help Center.
  </Card>

  <Card title="Help categories" href="/api-reference/help-categories/overview" icon="folder-tree" horizontal="false">
    Group articles inside a collection, and move categories between collections.
  </Card>

  <Card title="Help articles" href="/api-reference/help-articles/overview" icon="file-text" horizontal="false">
    Write, publish, search, and re-file articles, with their helpfulness and view counts.
  </Card>
</Columns>

Two behaviors are worth knowing before you start:

- **Every Help Center endpoint requires the Help Center on your plan** — reads as well as writes, unlike the dashboard, which keeps existing content readable after a downgrade.
- **Ordering is dashboard-only.** New collections, categories, and articles are appended to the end; rearranging them uses drag-and-drop, which has no API equivalent.

Authoring through the API behaves exactly like authoring in the dashboard — the same slug rules, the same publish timestamps, and the same cascading deletes.
