...
<img src="https://raw.githubusercontent.com/hvemasan/documentationai-starter-kit/main/images/logos/zapier.svg" width="80" height="80" alt="Zapier" />


## Where to find it

<Card title="Open Zapier in ProductBridge" href="https://app.productbridge.io/dashboard/settings/integrations" icon="zap" horizontal="true">
  Sidebar → **Connect Sources** → find **Zapier** in the catalog
</Card>
...
**In-app path:** Sidebar → **Connect Sources** → find **Zapier** in the catalog
...
- A ProductBridge Zapier API key (generated in the steps below)
</Callout>
...
<Steps>
  <Step title="Generate your API key" icon="key">
    In ProductBridge, open **Connect Sources > Zapier**, give the key a name, and click **Generate** to create a Zapier API key. Copy it right away — the full key (prefixed `zpk_`) is shown only once.

    Each API key is scoped to your organization and grants access to all boards within it.

    <Callout kind="alert">
      Keep your API key secure. Do not share it publicly or commit it to version control. You can revoke it and generate a new one at any time from the same page, but existing Zaps will need to be reconnected.
    </Callout>
  </Step>
...
<ExpandableGroup>
  ...
  <Expandable title="Authentication errors" default-open="false">
    - Generate a fresh API key in **ProductBridge > Connect Sources > Zapier** and check it starts with `zpk_`
  ...
  <Expandable title="Missing dropdown options" default-open="false">
    ...
    - Ensure your API key belongs to the organization containing the boards you expect to see
  </Expandable>
</ExpandableGroup>
