...
<Image src="/images/sso_redirect_tab_initial_setup_framed.png" width="960" height="540" alt="ProductBridge Settings → SSO page showing the SSO Redirect tab with the three-step setup wizard: redirect URL field, Test Redirect Flow button, and the Enable SSO toggle disabled until the test passes" />
...
<Image src="/images/sso_redirect_tab_configured_and_active_framed.png" width="960" height="540" alt="SSO Redirect tab fully configured and active: redirect URL set to https://test.resumeup.ai/chrome-extension-sign-in, the Tested badge is green, Enable Single Sign-On Redirect toggle is on, and Disable ProductBridge login toggle is off" />
...
<Image src="/images/sso_test_redirect_flow_in_progress_framed.png" width="960" height="500" alt="SSO test in progress: the SSO Redirect tab shows the Testing… state while a popup window opens to the customer login page (resumeup.ai) waiting for authentication and JWT callback" />
...
Go to **Settings → Security → API Keys**. If no key exists yet, click **Generate Private Key**.

<Image src="/images/sso_api_keys_tab_empty_state_framed.png" width="960" height="540" alt="API Keys tab in its empty state showing a No Key badge and a Generate Private Key button — the starting point before any SSO key has been created" />

After generation, the full secret is shown **once**. Copy it immediately and store it in your server environment.

<Image src="/images/sso_api_keys_tab_with_active_key_framed.png" width="960" height="540" alt="API Keys tab with an active key: the key value is masked as 7c879659●●●●●, an Active badge and Regenerate button are visible, and an alert reminds you to copy the key now because it cannot be viewed again" />
...
<Image src="/images/sso_widget_auth_tab_settings_framed.png" width="960" height="540" alt="Widget Auth tab showing three controls: Require identify() for widget toggle, Require user hash for identify calls toggle with the userHash = HMAC-SHA256(user.id, privateKey) formula displayed, and an Identify Mode selector with Upsert and Update Only options" />
...
<Image src="/images/sso_portal_tab_urls_framed.png" width="960" height="540" alt="Portal tab showing two read-only fields with copy buttons: Your Portal URL (the public-facing portal address) and SSO Callback URL (https://app.productbridge.io/api/redirects/sso) where your login page must redirect users back to with the ssoToken parameter" />
