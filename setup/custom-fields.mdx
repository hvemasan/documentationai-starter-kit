...
| **Select** | Single-choice dropdown | Urgency, Region |
| **Multi-Select** | Multiple choices from a list | Affected products, Personas |

<Callout kind="info">
  More field types (date, URL, user reference) are on the roadmap. If you need a specific type today, let us know.
</Callout>
...
<Steps>
  ...
  <Step title="Add a new field" icon="plus">
    ...
    - **Field type** — Text, Number, Select, or Multi-Select
    - **Placeholder** *(optional)* — helper text shown in the input
    - **Optional** — toggle off to make the field required (default: optional)
    - **Options** *(Select & Multi-Select only)* — define the choices users can pick from
  </Step>
...
- **Use Select for known sets.** If the value is one of a small fixed list (Urgency, Region, Persona), use Select — it forces consistency and makes filtering reliable.
