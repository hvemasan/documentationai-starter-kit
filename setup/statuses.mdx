...
<Card title="Open Manage Statuses" href="https://app.productbridge.io/dashboard/settings/statuses" icon="circle-dashed" horizontal="true">
  Sidebar → **Settings** → Modules → **Statuses**
</Card>

**Direct link:** [https://app.productbridge.io/dashboard/settings/statuses](https://app.productbridge.io/dashboard/settings/statuses)

**In-app path:** Sidebar → **Settings** → Modules → **Statuses** (the page is titled **Manage Statuses**)

**Requires:** `status:view` to read, `status:manage` to add, reorder, or delete.
...
Statuses are **organization-wide** and **shared across feedback and roadmap posts**. You can hide specific statuses from specific surfaces (e.g., "In Progress" on roadmap only, "Open" on feedback only). Changelog posts don't use statuses — they have their own labels system, managed at [**Settings → Changelog**](https://app.productbridge.io/dashboard/settings/changelog).
...
<Steps>
  <Step title="Open Statuses" icon="circle-dot">
    Go to [**Settings → Statuses**](https://app.productbridge.io/dashboard/settings/statuses) (the page is titled **Manage Statuses**).
  </Step>
...
- Outbound webhooks
- Linked external systems (Jira, Linear, ClickUp, GitHub) — if you've configured [status sync rules](/integrations/project-management/jira)
