...
<ParamField body="type" param-type="string" required="false" deprecated="false">
  Filter to entries with this single classification. One of `"fixed"`, `"new"`, `"improvement"`.
</ParamField>
...
| `422` | Validation error envelope | `type` is not a recognised classification, `limit` out of range, malformed UUIDs in `label_ids`. |
