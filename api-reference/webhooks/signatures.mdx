...
…and sends it in the `X-ProductBridge-Signature` header. The signing secret is the per-webhook value returned exactly once when you registered the webhook (the **Signing secret** modal shown when you add an endpoint under [**Settings → Connect → API & Webhooks → Webhook endpoints**](https://app.productbridge.io/dashboard/settings/api-webhooks)).
...
<Steps>
  <Step title="Read the raw request body" icon="binary" title-type="p">
  ...
  <Step title="Compute the expected signature" icon="key" title-type="p">
    `expected = "sha256=" + HMAC-SHA256(your_stored_secret, raw_body).hex()`
  </Step>

  <Step title="Compare in constant time" icon="shield-check" title-type="p">
  ...
  <Step title="Reject mismatches with HTTP 401" icon="x" title-type="p">
...
<Steps>
  <Step title="Revoke the compromised webhook" icon="trash-2" title-type="p">
  ...
  <Step title="Create a fresh webhook with the same URL" icon="plus" title-type="p">
  ...
  <Step title="Update your receiver's stored secret" icon="key" title-type="p">
    Replace the old `PRODUCTBRIDGE_WEBHOOK_SECRET` value with the new one in your secrets manager / env vars and redeploy.
  </Step>
</Steps>

<Callout kind="tip">
  Plan rotation as a deploy-coordinated step: revoke + recreate + update receiver are not atomic, and ProductBridge does not redeliver failed webhooks. Any events that occur between revoking the old webhook and the new secret being live in your receiver are dropped permanently — keep the window as short as possible, and re-fetch affected resources via the public API afterwards if you need to backfill.
</Callout>
