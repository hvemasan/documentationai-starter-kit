...
…and sends it in the `X-ProductBridge-Signature` header. The signing secret is the per-webhook value returned exactly once when you registered the webhook (the **Signing secret** modal shown when you add an endpoint under **Settings → Connect → API & Webhooks → Webhook endpoints**).
