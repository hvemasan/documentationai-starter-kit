...
Fires whenever a feedback post is created — including via the public API, dashboard, sidebar integrations (Zendesk, Freshdesk), and the AI processing pipeline. Roadmap post creation does not emit this event.
...
<Callout kind="info">
  This event does not fire when a post receives its *first* status (a transition from no status to a status). Only changes between two existing statuses emit `post.status_changed`.
</Callout>
