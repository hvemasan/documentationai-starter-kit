...
| `400` | Bad request | A semantic precondition failed (e.g. a referenced resource doesn't belong to your organization, parent comment doesn't belong to the same post). The body explains what's wrong. |
...
| `422` | Validation error | Pydantic rejected the request body — e.g. missing required field, wrong type, value out of range, or a model-level rule such as `merge_post_id == into_post_id`. The body lists every offending field. |
