...
| `400` | `{"detail": "Cannot merge posts. Ensure both posts exist, are active, and neither is already merged."}` | The merge could not be performed — e.g. one of the posts is inactive or already merged. Note: `detail` is a plain string here, not the usual `{"error": ...}` object. |
