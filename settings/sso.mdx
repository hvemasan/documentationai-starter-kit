...
When **Require identify() for widget** is enabled, anonymous users see a locked state in the widget instead of the default feedback form. The widget only accepts submissions from users who have been identified via `ProductBridge.init({ userToken })` or `ProductBridge.identify(token)`.
...
<Callout kind="alert" collapsed="false">
  The current ProductBridge widget SDK does not send a `user_hash` parameter with identify calls. Enabling this setting will cause identify calls made through the shipped SDK to be rejected. Leave it off unless you're integrating against the verify-identity API directly and sending `user_hash` yourself.
</Callout>
...
<ExpandableGroup>
  <Expandable title="Test says 'Token verification failed'" default-open="false">
    The JWT signature did not match. Check that the secret used to sign the token matches the API key displayed in **Settings → SSO → API Keys**. The key is only shown once — if you are unsure, generate a new one and update your server environment variable.
  </Expandable>
  ...
  <Expandable title="Widget shows 'Please log in' even after passing a userToken" default-open="false">
    The most common cause is a signature mismatch. Verify that `PRODUCTBRIDGE_WIDGET_SECRET` on your server matches the key in **Settings → SSO → API Keys**. Also confirm you are using the `HS256` algorithm — other algorithms are not accepted.
  </Expandable>
</ExpandableGroup>
